  ...
  methods: {
    submitLogin(){
      ...
      loginApi.submitLoginUser(this.user)
        .then(res=>{
          //第二步获取token字符串发到cookie
          //第一个参数Cookie名称,第二个参数值,第三个参数作用范围
          cookie.set('college_token', res.data.data.token,{domain:'localhost'})

          //第四步调用接口根据token获取用户信息,为了首页面显示
          loginApi.getLoginUserInfo()
            .then(res=>{
              this.loginInfo = JSON.stringify(res.data.data.userInfo)
              //获取返回用户信息,放到cookie里面
              cookie.set('college_ucenter',this.loginInfo  ,{domain:'localhost'})
              //跳转页面
